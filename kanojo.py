...
class KanojoManager(object):
	...
	def __init__(self, db=None, server=None, clothes_magic=51212494783, generate_secret=''):
		...
		if self.db and self.db.system.namespaces.find({ 'name': 'kanojos' }) is None:
			self.db.seqs.insert({
					'colection': 'kanojos',
					'id': 0
				})
	...
	def fill_fields(self, kanojo, self_user, owner_user=None):
		...
		kanojo['in_room'] = True
		if kanojo.get('profile_image_url')[0:4]!='http' and self.server:
	...
	def clear(self, kanojo, self_user, clear=CLEAR_SELF, check_clothes=False):
		if kanojo is None:
		...
		if check_clothes:
			...
			if changed:
				self.save(kanojo)
		self.fill_fields(kanojo, self_user)
	...
	def kanojos(self, kanojo_ids, self_user):
		query = {
			'id': {
				'$in': kanojo_ids
			}
		...
		for k in self.db.kanojos.find(query):
			arr.append(self.clear(k, self_user))

		# sort result
		rv = sorted(arr, key=lambda k: kanojo_ids.index(k['id']))
		return arr

	def kanojo(self, kanojo_id, self_user, clear=CLEAR_SELF, check_clothes=False):
		query = { 'id': kanojo_id }
		k = self.db.kanojos.find_one(query)
		if k:
			return self.clear(k, self_user, clear=clear, check_clothes=check_clothes)
		return k
