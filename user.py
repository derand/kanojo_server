...
class UserManager(object):
	...
	def __init__(self, db=None, server=None, kanojo_manager=None, store=None):
		...
		self.store = store
		self.last_uid = 1
		if self.db and self.db.system.namespaces.find({ 'name': 'users' }) is None:
			self.db.seqs.insert({
					'colection': 'users',
					'id': 0
				})
	...
	def default_user(self):
		return {"generate_count": 0, "description": "", "language": "ja", "level": 1, "kanojo_count": 0, "money": 100, "birth_month": 10, "stamina_max": 100, "facebook_connect": False, "profile_image_url": None, "sex": "no sure", "stamina": 100, "money_max": 100, "twitter_connect": False, "scan_count": 0, "birth_day": 25, "enemy_count": 0, "wish_count": 0, "id": 0, "name": None}
	...
	def give_present(self, user, kanojo, store_item_id):
		...
		if len(has_item) and has_item[0].get('units', 1) >= 1:
			...
			if store_item.has_key('clothes_item_id'):
				...
				self.update_if_kanojo_stolen(user, kanojo, action_dict.get('info', {}))
			...
			if not rv.get('info', {}).get('no_charge'):
	...
	def update_if_kanojo_stolen(self, user, kanojo, kanojo_love_increment_info, current_owner=None):
	...
	def user_action(self, user, kanojo, action_string=None, do_gift=None, do_date=None, is_extended_action=False, current_owner=None):
		...
		if user.get('level') < price.get('level', 0):
			return { "code": 403, "love_increment": { "alertShow": 1 }, "alerts": [ { "body": "You don't have enough level.", "title": "" } ] }
		...
		self.update_if_kanojo_stolen(user, kanojo, rv.get('info', {}), current_owner=current_owner)
