...
class ActivityManager(object):
    ...
    def create(self, activity_info):
        ...
        if activity_info.get('scanned'):
            activity['scanned'] = activity_info.get('scanned')

        if activity_info.get('activity'):
        ...
        if self._db:
            query = {
                '$or': [
                    {
                        'user': user_id,
                        'activity_type': { '$nin': [ACTIVITY_APPROACH_KANOJO, ACTIVITY_JOINED, ] },
                    },
                    {
                        'other_user': user_id,
    ...
    def all_activities(self, skip=0, limit=20, since_id=0):
        ...
        if self._db:
            query = {
                'activity_type': { '$nin': [ACTIVITY_APPROACH_KANOJO, ACTIVITY_SCAN, ] },
            }
            if since_id > 0:
